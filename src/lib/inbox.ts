...
function pickRecipient(
  ...
  const needle = tags.map((t) => t.trim().toLowerCase()).filter(Boolean);
...
async function dispatchEmail(payload: {
  ...
  try {
    ...
    return Boolean(json.dispatched);
  } catch {
    return false;
  }
}
...
export async function composeEmail(input: {
  ...
  const body = input.body.trim();
  if (!body) throw new Error("Message vide");
  const [contacts, companies, existing] = await Promise.all([
    fetchContacts(),
    fetchCompanies(),
  ...
  let conv =
    existing?.find(
      (c) =>
        (recipient.contact && c.contact_id === recipient.contact.id) ||
        (recipient.company && c.company_id === recipient.company.id)
    ) ?? null;
  if (!conv) {
    conv = await createConversation({
      title: subject,
      channel: "email",
      company_id: recipient.company?.id ?? null,
...
export async function remindDueVisaActivities(): Promise<void> {
  ...
  const due = rows.filter((a) => {
    const hay = `${a.subject} ${a.notes ?? ""}`.toLowerCase();
    return Boolean(a.due_at) && (hay.includes("visa") || hay.includes("echeance"));
  });
  ...
  await dispatchEmail({
    ...
    body: first.notes || first.subject,
  });
}
